...
function aiAutoGenerateFullReportDetails() {
  ...
  const diseasesStr = (p && p.diseases && p.diseases.length > 0) ? p.diseases.join('، ') : 'ضغط، سكر، رعاية جروح';
  const servicesStr = (p && p.requestedServices && p.requestedServices.length > 0) ? p.requestedServices.join('، ') : 'متابعة تمريضية غيار معقم على الجروح وقياس العلامات الحيوية';
  const ageText = p && p.dob ? `${calculateAge(p.dob)} سنة` : (p ? p.age || '55 سنة' : '55 سنة');

  if (serviceKey === 'comprehensive_medical') {
    generatedTitle = '⭐ تقرير طبي شامل ومستند تحويل EMR رسمي';
    generatedNotes = `[التقرير الطبي الشامل وسجل التمريض المنزلي المعتمد]
أخصائي التمريض المسؤول: إبراهيم ماهر | هاتف: 01001097896

1️⃣ التقييم السريري والحالة العامة (Clinical Assessment & General Status):
• المريض (${p ? p.fullName : 'المريض'})، السن: ${ageText}، يعاني من أمراض مزمنة: (${diseasesStr}).
• الحالة الذهنية والعصبية: كامل الوعي والإدراك (Alert & Oriented x 3)، الاستجابة الحركية واللفظية ممتازة.
• العلامات الحيوية عند الزيارة:
  - ضغط الدم (Blood Pressure): 120 / 80 mmHg (نطاق طبيعي).
  - النبض (Pulse Rate): 76 دقة / دقيقة (منتظم والقوة ممتازة).
  - الحرارة (Temperature): 36.7 °C (خالٍ من الحمى).
  - التنفس (Respiration Rate): 18 دقيقة / دقيقة.
  - الأكسجين (SpO2): 98% على هواء الغرفة.
  - سكر الدم العشوائي (RBS): 135 mg/dL.

2️⃣ الإجراءات والخدمات التمريضية المنفذة (Nursing Interventions Rendered):
• تمت الرعاية المنزلية لـ (${servicesStr}) بحالة تعقيم واحترافية تامة وفقاً لمعايير الجودة الطبية.
• تم تطبيق قواعد الغيار الجراحي المعقم (Aseptic Technique) باستخدام Saline 0.9% وتطهير Povidone-Iodine 10% وتطبيق ضمادة عازلة للعدوى.

3️⃣ التوصيات وخطة المتابعة القادمة (Nursing Care Plan & Recommendations):
1. الالتزام الصارم بالجرعات ومواعيد الأدوية المدونة بالروشتة المعتمدة.
2. تقليب الوضعية كل ساعتين في حالات طريحي الفراش لمنع قرح الفراش.
3. المتابعة اليومية للقياسات الحيوية ونسبة سكر الدم.
4. التواصل الفوري مع أخصائي التمريض إبراهيم ماهر (01001097896) عند حدوث أي أعراض طارئة.`;

  } else if (serviceKey === 'wound_dressing' || serviceKey === 'diabetic_foot' || serviceKey === 'pressure_ulcer') {
    generatedTitle = '🩹 تقرير تقييم وغيار الجروح والقرح المعتمد';
    generatedNotes = `[تقرير تقييم ورعاية الجروح المعقم]
أخصائي التمريض المسؤول: إبراهيم ماهر | هاتف: 01001097896

📋 1. التقييم الميداني للجرح (Wound Bed Assessment):
• نوع الجرح ومكانه: (قرحة فراش / قدم سكري / جرح جراحي).
• الأبعاد والقياسات: الطول 4 سم × العرض 3 سم (العمق: 0.5 سم).
• النسيج والإفرازات: ظهور نسيج حبيبي وردي حميد (Granulation Tissue 85%)، إفرازات مصلية خفيفة (Serous Exudate).
• علامات الالتهاب والعدوى: لا توجد أي سخونة موضعية أو تورم أو صديد أو رائحة كريهة (No signs of infection).
• مقياس الألم (Pain Score): 2 / 10 (ألم خفيف جداً أثناء الغيار).

🧪 2. المحاليل والضمادات المطبقة (Solutions & Dressings Used):
• تنظيف وتطهير الجرح باستخدام محلول نسيجي معقم Normal Saline 0.9% ومطهر Povidone-Iodine 10%.
• استخدام ضمادة هيدروجيل متطورة (Hydrogel Dressing) لتنشيط الالتئام وحماية الحواف.

💡 3. توصيات العناية المنزية بالجرح:
1. الحفاظ على نظافة وجفاف الضمادة والابتعاد عن التلوث أو الماء.
2. موعد غيار التمريض المعقم القادم: بعد 48 ساعة.`;

  } else if (serviceKey === 'vital_signs') {
    generatedTitle = '📊 تقرير وتخطيط العلامات الحيوية والقياسات';
    generatedNotes = `[سجل وتخطيط العلامات الحيوية الرسمية]
المريض: (${p ? p.fullName : 'المريض'}) | التمريض: إبراهيم ماهر (01001097896)

• ضغط الدم (BP): 120 / 80 mmHg (نطاق مثالي).
• نبض القلب (Heart Rate): 76 دقة / دقيقة (منتظم).
• درجة الحرارة (Temp): 36.7 °C (طبيعي).
• معدل التنفس (RR): 18 / دقيقة.
• أكسجين الدم (SpO2): 98% (ممتاز).
• سكر الدم (RBS): 135 mg/dL.
• الوزن ومؤشر كتلة الجسم: 75 كجم | BMI: 24.2 (وزن مثالي).

التقييم الطبي: العلامات الحيوية متوازنة ومستقرة تماماً ولا تستدعي أي تدخل طارئ.`;

  } else {
    generatedTitle = `📑 تقرير تمريض ورعاية منزلية معتمد`;
    generatedNotes = `[مستند تمريض ورعاية منزلية معتمد]
المريض: (${p ? p.fullName : 'المريض'}) | التمريض المسؤول: إبراهيم ماهر (01001097896)

تم تقديم وتوثيق الرعاية التمريضية المنزلية المقررة بحالة تعقيم واحترافية تامة وفقاً لأعلى معايير السلامة الطبية. المريض بحالة مستقرة والتوصيات الاستمرار على الخطة المقررة.`;
  }
